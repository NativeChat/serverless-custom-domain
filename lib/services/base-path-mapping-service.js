...
class BasePathMappingService extends ServerlessService {
	...
	async _createMappingAsync() {
		let apiName = this.provider.naming.getApiGatewayName();
		let apiId = await this._getApiIdAsync(apiName);

		if (!apiId) {
			apiName = this.provider.naming.getWebsocketsApiName();
			apiId = await this._getApiIdAsync(apiName);
		}
	...
	async _getApiIdV2Async(apiName, nextToken) {
		const limit = 500;
		const getApisRequest = { MaxResults: limit.toString() };
