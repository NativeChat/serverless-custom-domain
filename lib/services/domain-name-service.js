...
class DomainNameService extends ServerlessService {
	...
	async removeApiNameAsync() {
		...
		if (domainNameExists) {
			...
			if (this.config.apiGwVersion === ApiGwVersions.V2) {
				return this.provider.request(AwsServices.ApiGatewayV2, "deleteDomainName", { DomainName: domainName });
			} else {
				return this.provider.request(AwsServices.ApiGateway, "deleteDomainName", { domainName });
			}
		}
	...
	async _getDomainNameInfoAsync(domainName) {
		let domains = await this.provider.request(AwsServices.ApiGateway, "getDomainNames", {});
		...
		domains = await this.provider.request(AwsServices.ApiGatewayV2, "getDomainNames", {});
		if (!domains || !domains.Items) {
			return null;
		}
	...
	async _createDomainNameAsync(domainName) {
		...
		if (this.config.apiGwVersion === ApiGwVersions.V2) {
			return this._provider.request(AwsServices.ApiGatewayV2, "createDomainName", { DomainName: domainName, CertificateArn: certificateArn }).promise();
		}

		return this._provider.request(AwsServices.ApiGateway, "createDomainName", { domainName, certificateArn });
	}
}
