...
class DomainNameService extends ServerlessService {
	...
	async _createDomainNameAsync(domainName) {
		...
		if (this.config.apiGwVersion === ApiGwVersions.V2) {
			return this.provider.request(AwsServices.ApiGatewayV2, "createDomainName", { DomainName: domainName, CertificateArn: certificateArn }).promise();
		}

		return this.provider.request(AwsServices.ApiGateway, "createDomainName", { domainName, certificateArn });
	}
}
